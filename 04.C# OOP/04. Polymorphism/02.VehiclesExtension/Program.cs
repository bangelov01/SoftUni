   ...
   public class StartUp
    {
        static void Main(string[] args)
        {
            ...
            Vehicle car = new Car(double.Parse(info[1]), double.Parse(info[2]), double.Parse(info[3]));

            info = Console.ReadLine().Split();

            Vehicle truck = new Truck(double.Parse(info[1]), double.Parse(info[2]), double.Parse(info[3]));

            info = Console.ReadLine().Split();

            Vehicle bus = new Bus(double.Parse(info[1]), double.Parse(info[2]), double.Parse(info[3]));
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                try
                {
                    ...
                    else if (actions[1] == nameof(Bus))
                    {
                        DoAction(bus, actions[0], double.Parse(actions[2]));
                    }
                }
                catch (Exception ex)
                {
